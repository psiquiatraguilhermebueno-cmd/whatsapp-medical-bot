...
ADMIN_PHONE_NUMBER = os.getenv('ADMIN_PHONE_NUMBER') or "14997799022"
...
Me diga qual você prefere respondendo com o horário (ex.: "{available_times[1]}") e já deixamos combinado! Um ótimo dia!"""
        ...
        if success:
            ...
            return jsonify({"status": "error", "message": "Falha ao enviar teste imediato"}), 500
            
    except Exception as e:
        print(f"💥 Error sending immediate test: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500
